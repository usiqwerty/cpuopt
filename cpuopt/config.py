...
from cpuopt.configo.config_event_handler import ConfigEventHandler
from cpuopt.cpufreqctl import get_freq_max, get_freq_min, get_frequency_max_limit, get_frequency_min_limit

# from auto_cpufreq.sysparam import AVAILABLE_GOVERNORS_SORTED
...
config_defaults = {
    "charger": {
        ...
        "scaling_min_freq": get_freq_min(),
        "scaling_max_freq": get_freq_max(),
        "turbo": "auto"
    },
    ...
    "battery": {
        "governor": "power", #AVAILABLE_GOVERNORS_SORTED[-1]
...
def find_config_file(args_config_file) -> str:
    ...
    if args_config_file is not None:  # (1) Command line argument was specified
        ...
        return user_config_file  # (2) User config file
    else:
        return system_config_file  # (3) System config file (default if nothing else is found)
...
class _Config:
    def __init__(self) -> None:
        ...
        self._config.read_dict(config_defaults)
    ...
    def validate_config(self):
        for power_supply in {"charger", "battery"}:
            for freq_type in {"scaling_min_freq", "scaling_max_freq"}:
                if not phys_min_limit <= int(self._config[power_supply][freq_type]) <= phys_max_limit:
                    print(f"Given value for '{freq_type}' is not within the "
                          f"allowed frequencies {phys_min_limit}-{phys_max_limit} kHz")
                    exit(1)
